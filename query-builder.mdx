...
const posts = await Post.search('typescript').get()

// Case-sensitive search
const results = await Article.search('TypeScript', true).get()
```

<Warning>
  The `search()` method requires a text index on your collection. See [MongoDB text indexes](https://www.mongodb.com/docs/manual/core/indexes/index-types/index-text/) for setup instructions.
</Warning>
